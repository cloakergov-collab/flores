...
/**
 * Servidor do mirror "Paraíso das Flores" + proxy de pagamento PIX (AssetPay).
 *
 * - Serve os arquivos estáticos da pasta ./site
 * - Expõe /api/gerar  (POST) -> cria uma transação PIX na AssetPay
 * - Expõe /api/status (GET)  -> consulta o status da transação na AssetPay
 *
 * A chave secreta NUNCA vai para o navegador: ela fica só aqui no servidor.
 * Sem chaves configuradas, roda em MODO DEMO (gera um PIX fake que "paga"
 * sozinho após alguns segundos) para você testar o fluxo visual.
 *
 * Configuração (variáveis de ambiente):
 *   ASSETPAY_SECRET_KEY   chave secreta (username do Basic Auth)
 *   ASSETPAY_PUBLIC_KEY   chave pública (password do Basic Auth)
 *   ASSETPAY_BASE_URL     opcional (default: https://api.assetpay.com.br/api/v1)
...
// Faz a chamada HTTPS à AssetPay com Basic Auth.
function assetpayRequest(method, endpoint, payload) {
  return new Promise((resolve, reject) => {
...
async function handleGerar(req, res) {
  ...
  try {
    ...
    if (r.status === 201 && r.body && r.body.pix) {
      return sendJson(res, 200, {
        success: true,
        transactionId: r.body.id,
        pixCode: r.body.pix.qrcode,
        qrcodeUrl: r.body.pix.qrcodeUrl ||
          ("https://api.qrserver.com/v1/create-qr-code/?size=240x240&data=" +
            encodeURIComponent(r.body.pix.qrcode)),
        expiresAt: r.body.pix.expirationDate || null
      });
    }
    // Log de diagnóstico: mostra exatamente o que a AssetPay respondeu.
    console.error("\n[AssetPay] /transactions falhou");
...
async function handleStatus(req, res, query) {
  ...
  // ---------- MODO REAL ----------
  try {
    const r = await assetpayRequest("GET", "/transactions/" + encodeURIComponent(id), null);
    const status = (r.body && r.body.status) || "unknown";
    return sendJson(res, 200, {
      success: true,
      status: status,
      paid: status === "paid"
...
server.listen(PORT, () => {
  ...
  if (DEMO_MODE) {
    console.log("  ⚠️  PIX em MODO DEMO (sem chaves AssetPay).");
    console.log("      O pagamento confirma sozinho ~18s após gerar o QR.");
    console.log("      Para usar de verdade, defina ASSETPAY_SECRET_KEY e ASSETPAY_PUBLIC_KEY.\n");
  } else {
    console.log("  ✅ PIX em MODO REAL via AssetPay (" + BASE_URL + ").\n");
  }
});
